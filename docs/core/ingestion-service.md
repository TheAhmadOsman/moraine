...
`moraine-ingest` is the sole writer that transforms JSONL lines from configured Codex, Claude Code, and Hermes ShareGPT trajectory sources into canonical event rows. It owns file-change detection, append-safe consumption, schema-aware normalization, and batched persistence with checkpoint advancement. Although retrieval runs elsewhere, this service defines the invariants retrieval depends on: stable event identity and source provenance. [src: rust/ingestor/src/main.rs:L7, rust/ingestor/src/ingestor.rs:L45, rust/ingestor/src/normalize.rs:L336, config/moraine.toml:L21-L42]
